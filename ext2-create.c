...
void write_block_group_descriptor_table(int fd)
{
	...
	block_group_descriptor.bg_used_dirs_count = 2; // root directory
...
void bitmap_helper(u8 *map_value, int last_block, int size)
{
...
void write_block_bitmap(int fd)
{
	...
	bitmap_helper(map_value, LAST_BLOCK, BLOCK_SIZE - 1);
...
void write_inode_table(int fd)
{
	...
	memcpy(hello_symlink_inode.i_block, "hello-world", 11);
